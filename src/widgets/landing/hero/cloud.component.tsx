...
const CloudComponent = () => {
  ...
  const cloudName = (): {
    cloudType: cloudType;
    additionalIcon?: additionalIconType;
    isPuddle?: boolean;
  } => {
  ...
  return (
    <button
      className={clsx(
        "absolute -top-6 left-[18.2rem] t-xs:left-[7rem] t-xs:-top-4 w-6 lg:w-7 lg:h-7 2xl:w-10 2xl:h-10 justify-center items-center flex lg:hover:scale-125 duration-150",
        shouldAnimate && "animate-hero-icons-fade-in"
      )}
      onClick={toggleCloudAnim}
    >
      <IconComponent
        name={cloudName().cloudType}
        className={clsx(iconClassnameByTheme, "w-full h-full")}
      />
      {cloudName().additionalIcon && (
        <IconComponent
          ...
          className={clsx(
            "absolute",
            cloudName().additionalIcon === "lightning"
              ? `top-1/2 ${iconClassnameByTheme}`
              : "top-[70%]"
          )}
        />
      )}
      {cloudName().isPuddle && (
        <IconComponent name="puddle" className={clsx("absolute top-full mt-1.5", scaleClass())} />
      )}
